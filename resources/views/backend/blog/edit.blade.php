        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-body p-4">
              <h5 class="mb-4">Edit BLog Post
              </h5>
              <form action="{{ route('blog.post.update', $post->id ) }}" method="post" class="row g-3" enctype="multipart/form-data">
                @csrf
                @method('PAtCH')
                ...
                <div class="col-md-6">
                  ...
                  <input type="text" class="form-control @error('post_title') is-invalid @enderror" 
                         name="post_title" id="post_title" value="{{ old('post_title',  $post->post_title) }}">
                         @error('post_title')
                           <p class="mb-0 text-danger">{{ $message }}</p>
                         @enderror
                </div>
                ...
                <div class="col-md-12">
                  ...
                  <textarea class="form-control @error('short_desc') is-invalid @enderror" 
                            id="short_desc" name="short_desc"  rows="3">{{ old('short_desc',  $post->short_desc) }}</textarea>
                            @error('short_desc')
                            <p class="mb-0 text-danger">{{ $message }}</p>
                          @enderror
                </div>
                ...
                <div class="col-md-12">
                  ...
                  <textarea class="form-control @error('long_desc') in-invalid @enderror" 
                            name="long_desc" id="myeditorinstance" rows="3">{!! old('long_desc', $post->long_desc) !!}</textarea>
                            @error('long_desc')
                              <p class="mb-0 text-danger">{{ $message }}</p>
                            @enderror
                </div>
                ...
                <div class="col-md-6">
                  <img src="{{ asset($post->post_image) }}" alt="Post Image" id="showImage" class="p-1 w-100">
                </div>
